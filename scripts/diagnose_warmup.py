    ...
    for conn in connectivity_levels:
        for algo in [AlgorithmType.ADAPTIVE, AlgorithmType.BASELINE]:
            ...
            rows.append({
                ...
                "delivery_rate_std": np.std(delivery_rates, ddof=1) if len(delivery_rates) > 1 else 0,
                "assignment_rate_mean": np.mean(assignment_rates),
                "assignment_rate_std": np.std(assignment_rates, ddof=1) if len(assignment_rates) > 1 else 0,
                "response_time_mean": np.mean(response_times) if response_times else None,
                "n": runs_per_config,
            })
...
def plot_predictability_histogram(p_values, connectivity, warmup_s, save_path=None):
    ...
    if nonzero:
        ...
        ax2.text(0.5, 0.5, "No nonzero P values", transform=ax2.transAxes,
...
def plot_algorithm_comparison(rows, warmup_s, runs, save_path=None):
    ...
    conn_levels = sorted(set(r["connectivity"] for r in rows), reverse=True)
    ...
    for ax, metric, label in [
        ...
        bars_a = ax.bar(
            ...
            yerr=adaptive_errs, capsize=4,
            label="Adaptive", color=COLORS["adaptive"], alpha=0.85,
        )
        bars_b = ax.bar(
...
def main():
    ...
    parser.add_argument(
        "--connectivity", type=float, nargs="*", default=[0.75, 0.40, 0.20],
    ...
    print(f"  Nonzero P values:      {engine.warmup_nonzero} "
    ...
    rows = quick_comparison(
        config, args.connectivity, args.runs, args.seed
    )
    ...
    for conn in sorted(set(r["connectivity"] for r in rows), reverse=True):
        a = next(r for r in rows if r["connectivity"] == conn and r["algorithm"] == "adaptive")
        b = next(r for r in rows if r["connectivity"] == conn and r["algorithm"] == "baseline")
