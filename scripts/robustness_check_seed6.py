...
Usage:
    python scripts/robustness_check_seed6.py
"""
...
def print_stats(label, values):
    arr = np.array(values)
    print(
        f"  {label}: mean={arr.mean():.4f}  std={arr.std(ddof=1):.4f}  "
...
def main():
    ...
    print(
        f"Done. {len(adaptive_results)} Adaptive + {len(baseline_results)} Baseline runs.\n"
    )
    ...
    if not catastrophic_seeds:
        ...
        for seed, result, engine in catastrophic_seeds:
            ...
            print(
                f"    failed_assignments = {engine._manager.coordinator.statistics['failed_assignments']}"
            )
            ...
            for cid in coord_ids:
                p_above = sum(
                    1 for mid in mobile_ids if matrix.get_predictability(cid, mid) > 0.3
                )
    ...
    ada_rates_excl = [
        r.assignment_rate
        for s, r, _ in adaptive_results
        if s not in catastrophic_seed_set
    ]
    bas_rates_excl = [
        r.assignment_rate
        for s, r, _ in baseline_results
        if s not in catastrophic_seed_set
    ]
    ...
    if catastrophic_seeds:
        ...
        print(
            f"  Adaptive@20% assignment_rate = {ada_mean_excl:.4f} +/- {ada_std_excl:.4f}"
        )
        print(
            f"  Baseline@20% assignment_rate = {bas_mean_excl:.4f} +/- {bas_std_excl:.4f}"
        )
        ...
        if ada_mean_excl == bas_mean_excl and ada_std_excl == bas_std_excl:
            ...
            print("     between Adaptive and Baseline, confirming that the P > 0.3")
            print("     filter is inactive for all other seeds at 20%.")
    else:
