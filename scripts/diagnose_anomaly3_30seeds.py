...
Runs 30 seeds for both Adaptive and Baseline at 20% connectivity.
For each seed, reports assignment rate, failed assignments, number of
coord-mobile pairs with P > 0.3, and mean response time.  Highlights
seeds where Adaptive assignment_rate drops below 0.85.

Usage:
    python scripts/diagnose_anomaly3_30seeds.py
"""
...
def main():
    ...
    print(
        f"{'seed':>5}  {'assigned':>8}  {'total':>5}  {'rate':>7}  {'failed':>6}  {'P>0.3':>5}  {'rt_mean':>8}"
    )
    ...
    print("\n\n--- COMPARISON ---")
    print(
        f"Assignment Rate Adaptive:  {np.mean(adaptive_rates):.4f} ± {np.std(adaptive_rates, ddof=1):.4f}"
    )
    print(
        f"Assignment Rate Baseline:  {np.mean(baseline_rates):.4f} ± {np.std(baseline_rates, ddof=1):.4f}"
    )
    print(
        f"Response Time Adaptive:    {np.mean(adaptive_rt):.4f} ± {np.std(adaptive_rt, ddof=1):.4f}"
    )
    print(
        f"Response Time Baseline:    {np.mean(baseline_rt):.4f} ± {np.std(baseline_rt, ddof=1):.4f}"
    )

    # Show the outlier seeds
    print("\n--- Seeds where Adaptive assignment_rate < 0.85 ---")
    for i, (seed, rate) in enumerate(zip(seeds, adaptive_rates, strict=False)):
        if rate < 0.85:
            print(
                f"  seed={seed}, rate={rate:.4f}, baseline_rate={baseline_rates[i]:.4f}"
            )
