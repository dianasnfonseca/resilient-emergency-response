...
if TYPE_CHECKING:
    ...
    from ercs.evaluation.metrics import EvaluationReport, MetricType
...
def compute_summary_stats(
    ...
    for alg in ["adaptive", "baseline"]:
        ...
        for conn in sorted(df["connectivity"].unique()):
            values = df[
                (df["algorithm"] == alg) & (df["connectivity"] == conn)
            ][metric_col].dropna()
            n = len(values)
            if n == 0:
...
def build_parameter_tables(
    ...
    network_data = [
        ("Node count", f"{config.total_nodes} ({config.network.coordination_node_count} coordination + {config.network.mobile_responder_count} mobile)", "Ullah & Qayyum (2022)"),
        ("Simulation area", f"{config.network.simulation_area.width_m:.0f} x {config.network.simulation_area.height_m:.0f} m\u00b2", "Ullah & Qayyum (2022)"),
        ("Incident zone", f"{config.network.incident_zone.width_m:.0f} x {config.network.incident_zone.height_m:.0f} m\u00b2", "Ullah & Qayyum (2022)"),
        ("Radio range", f"{config.network.radio_range_m:.0f} m", "Ullah & Qayyum (2022)"),
        ("Buffer size", f"{config.network.buffer_size_bytes / 1_048_576:.0f} MB", "Ullah & Qayyum (2022)"),
        ("Message size", f"{config.network.message_size_bytes / 1_000:.0f} kB", "Kumar et al. (2023)"),
        ("Connectivity scenarios", ", ".join(f"{c*100:.0f}%" for c in config.network.connectivity_scenarios), "Karaman et al. (2026)"),
        ("Mobility model", config.network.mobility_model.value.replace("_", " ").title(), "Ullah & Qayyum (2022)"),
        ("Speed range", f"{config.network.speed_min_mps:.0f}\u2013{config.network.speed_max_mps:.0f} m/s", "Ullah & Qayyum (2022)"),
    ]
    ...
    scenario_data = [
        ("Task arrival", config.scenario.message_generation_model.title(), "Pu et al. (2025)"),
        ("Message rate", f"{config.scenario.message_rate_per_minute} msgs/min", "Kumar et al. (2023)"),
        ("Urgency distribution", f"{config.scenario.urgency_distribution.high*100:.0f}% H / {config.scenario.urgency_distribution.medium*100:.0f}% M / {config.scenario.urgency_distribution.low*100:.0f}% L", "Li et al. (2025)"),
        ("Simulation duration", f"{config.scenario.simulation_duration_seconds} s ({config.scenario.simulation_duration_seconds // 60} min)", "Ullah & Qayyum (2022)"),
        ("Runs per configuration", f"{config.scenario.runs_per_configuration}", "Law (2015)"),
    ]

    coordination_data = [
        ("Update interval", f"{config.coordination.update_interval_seconds // 60} min", "Kaji et al. (2025)"),
        ("Priority levels", f"{config.coordination.priority_levels}", "Rosas et al. (2023)"),
        ("Path threshold", f"P > {config.coordination.available_path_threshold}", "Ullah & Qayyum (2022)"),
        ("Adaptive ordering", config.coordination.adaptive_task_order.replace("_", " ").title(), "Kaji et al. (2025)"),
        ("Baseline ordering", config.coordination.baseline_task_order.upper(), "Design decision"),
    ]
...
def build_ttest_table(report: EvaluationReport) -> pd.DataFrame:
    ...
    for comp in report.comparisons:
        records.append({
            ...
            "Connectivity": f"{comp.connectivity_level * 100:.0f}%" if comp.connectivity_level is not None else "Overall",
...
def plot_grouped_bars(
    ...
    bars1 = ax.bar(
        x - width / 2, adaptive_means, width,
        yerr=adaptive_cis, capsize=3,
        label="Adaptive", color=COLORS["adaptive"],
        edgecolor="white", linewidth=0.5,
        error_kw={"linewidth": 1},
    )
    bars2 = ax.bar(
        x + width / 2, baseline_means, width,
        yerr=baseline_cis, capsize=3,
    ...
    ax.set_xticks(x)
    ax.set_xticklabels([CONNECTIVITY_LABELS.get(c, f"{c*100:.0f}%") for c in conn_levels])
    ax.legend()
...
def plot_box_distributions(
    ...
    fig, axes = plt.subplots(1, n_metrics, figsize=(5 * n_metrics, 5), constrained_layout=True)
    if n_metrics == 1:
        axes = [axes]
    ...
    for ax, metric_key in zip(axes, metrics):
        ...
        for conn in conn_levels:
            for alg in ["adaptive", "baseline"]:
                ...
                labels.append(f"{ALGORITHM_LABELS[alg][:4]}.\n{CONNECTIVITY_LABELS.get(conn, f'{conn*100:.0f}%')}")
                colors_list.append(COLORS[alg])

        bp = ax.boxplot(
            data_groups, patch_artist=True, labels=labels,
...
def plot_heatmap(
    ...
    metrics = metrics or list(METRICS_CONFIG.keys())
    n_metrics = len(metrics)
    fig, axes = plt.subplots(1, n_metrics, figsize=(5 * n_metrics, 3), constrained_layout=True)
    if n_metrics == 1:
        axes = [axes]
    ...
    for ax, metric_key in zip(axes, metrics):
        ...
        for i in range(len(algorithms)):
            for j in range(len(conn_levels)):
                fmt = cfg["fmt"]
                text_color = "white" if matrix[i, j] > (matrix.max() + matrix.min()) / 2 else "black"
...
def plot_degradation_lines(
    ...
    for ax, metric_key in zip(axes, metrics):
        ...
        for alg in ["adaptive", "baseline"]:
            ...
            ax.plot(x, means, "o-", color=COLORS[alg], label=ALGORITHM_LABELS[alg],
...
def save_figure(fig: Figure, filename: str, output_dir: str | Path = "outputs/figures") -> Path:
