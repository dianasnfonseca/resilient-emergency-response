...
class SimulationResults:
    ...
    def system_availability(self) -> float | None:
        ...
        if self.total_coordination_cycles == 0:
            return None
        return (
            self.active_coordination_cycles / self.total_coordination_cycles
        ) * 100.0
...
class ExperimentRunner:
    ...
    - Progress tracking
    - Seed validation via ``configs/valid_seeds.json``

    When ``use_valid_seeds=True`` (default), the runner loads pre-screened
    seeds from ``configs/valid_seeds.json`` (generated by
    ...
    def __init__(
        ...
        Args:
            ...
            use_valid_seeds: Whether to use pre-screened seeds from
                ``configs/valid_seeds.json`` (default True)
        """
        self.config = config or SimulationConfig()
    ...
    def _load_valid_seeds(self) -> list[int] | None:
        """Load pre-screened seeds from configs/valid_seeds.json.
