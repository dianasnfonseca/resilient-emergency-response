...
class ExperimentRunner:
    ...
    - Progress tracking
    - Seed validation via ``config/valid_seeds.json``

    When ``use_valid_seeds=True`` (default), the runner loads pre-screened
    seeds from ``config/valid_seeds.json`` (generated by
    ``scripts/validate_seeds.py``).  Each seed is guaranteed to produce a
    topology where at least one transport/liaison node can reach a
    coordination node at every connectivity level.
    ...
    Attributes:
        ...
        use_valid_seeds: Whether to load pre-screened seeds
    """

    # Default path relative to project root (src/ercs/simulation/engine.py → ../../../../config/)
    _VALID_SEEDS_PATH = (
        Path(__file__).resolve().parent.parent.parent.parent / "config" / "valid_seeds.json"
    )
    ...
    def __init__(
        ...
        Args:
            ...
            use_valid_seeds: Whether to use pre-screened seeds from
                ``config/valid_seeds.json`` (default True)
        """
        self.config = config or SimulationConfig()
        self.base_seed = base_seed
        self.use_valid_seeds = use_valid_seeds
        self._results: list[SimulationResults] = []
        self._valid_seeds: list[int] | None = None

        if use_valid_seeds:
            self._valid_seeds = self._load_valid_seeds()

    def _load_valid_seeds(self) -> list[int] | None:
        """Load pre-screened seeds from config/valid_seeds.json.

        Returns:
            List of valid seed integers, or None if file not found.
        """
        path = self._VALID_SEEDS_PATH
        if not path.exists():
            return None
        ...
        seeds = data.get("valid_seeds", [])
        return [int(s) for s in seeds] if seeds else None

    def _get_seeds(self, count: int) -> list[int]:
        """Get a list of seeds for experiment runs.

        If valid seeds are loaded, returns the first ``count`` seeds from
        the pre-screened list.  Otherwise falls back to sequential seeds
        starting from ``base_seed``.

        Args:
            count: Number of seeds needed.

        Returns:
            List of integer seeds.
        """
        if self._valid_seeds is not None and len(self._valid_seeds) >= count:
            return self._valid_seeds[:count]

        # Fallback: sequential seeds
    ...
    def run_all(
        ...
        seeds = self._get_seeds(runs_per_config)
