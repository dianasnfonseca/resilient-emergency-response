...
class TestNodeClass:
    ...
    def test_distance_same_location(self):
        ...
        node_a = Node(
            "a", NodeType.MOBILE_RESPONDER, 100.0, 100.0, True, BUFFER_SIZE_BYTES
        )
        node_b = Node(
            "b", NodeType.MOBILE_RESPONDER, 100.0, 100.0, True, BUFFER_SIZE_BYTES
        )
    ...
    def test_within_range_true(self):
        ...
        node_a = Node("a", NodeType.MOBILE_RESPONDER, 0.0, 0.0, True, BUFFER_SIZE_BYTES)
        node_b = Node(
            "b", NodeType.MOBILE_RESPONDER, 50.0, 0.0, True, BUFFER_SIZE_BYTES
        )

        # 50m apart, 100m range
        assert node_a.is_within_range(node_b, 100.0)

    def test_within_range_false(self):
        """Test nodes outside communication range."""
        node_a = Node("a", NodeType.MOBILE_RESPONDER, 0.0, 0.0, True, BUFFER_SIZE_BYTES)
        node_b = Node(
            "b", NodeType.MOBILE_RESPONDER, 150.0, 0.0, True, BUFFER_SIZE_BYTES
        )

        # 150m apart, 100m range
        assert not node_a.is_within_range(node_b, 100.0)

    def test_within_range_exact_boundary(self):
        """Test nodes exactly at communication range boundary."""
        node_a = Node("a", NodeType.MOBILE_RESPONDER, 0.0, 0.0, True, BUFFER_SIZE_BYTES)
        node_b = Node(
            "b", NodeType.MOBILE_RESPONDER, 100.0, 0.0, True, BUFFER_SIZE_BYTES
        )
...
class TestTopologyGenerator:
    ...
    def test_coordination_nodes_in_coordination_zone(
...
class TestConnectivityScenarios:
    ...
    def test_generate_produces_full_connectivity(self, generator: TopologyGenerator):
    ...
    def test_generate_ignores_legacy_connectivity_kwarg(self):
        ...
        topo_legacy = generate_topology(connectivity_level=0.5, random_seed=42)
    ...
    def test_link_availability_varies_with_seed(self):
        ...
        hashes = {zlib.crc32(f"{pair}|{seed}".encode()) % 10000 for seed in range(100)}
    ...
    def test_lower_connectivity_is_strict_subset(self):
        ...
        available_75 = {p for p, h in zip(pairs, hashes, strict=False) if h < 7500}
        available_40 = {p for p, h in zip(pairs, hashes, strict=False) if h < 4000}
        available_20 = {p for p, h in zip(pairs, hashes, strict=False) if h < 2000}
...
class TestZoneBoundaryConditions:
    ...
    def test_zones_do_not_overlap(self):
        ...
        assert (
            incident_right < coord_left
            or coord.origin_x + coord.width_m < incident.origin_x
        )
...
class TestGraphProperties:
    ...
    def test_node_attributes_in_graph(self):
        ...
        for _node_id, data in topology.graph.nodes(data=True):
